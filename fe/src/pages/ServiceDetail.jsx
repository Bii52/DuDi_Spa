    ...
    <div className="flex flex-col w-[90%] mx-auto">
        <div id='top' className="">
            <div className="flex justify-between">                
              ...
              <span className='w-[20%]'>{service.price}</span>
            </div>
            <div className="flex justify-between">
                <div className="flex gap-1">
                    ...
                    {Array.from({ length: 5 }).map((_, index) => (
                        ...
                        key={index}
                        className={
                            index + 1 <= Math.floor(service.rating)
                            ? 'ri-star-fill'
                            : index + 0.5 <= service.rating
                            ? 'ri-star-half-line'
                            : 'ri-star-line'
                        }
                        />
                    ...
                    <span className='flex gap-1 text-black'>({service.reviewsCount}) </span> 
                    </span>
                   
                </div>
                <Link to={`/booking/${id}`} className='w-[20%]'>
                  <button className='w-full !bg-[#BA7894] rounded-xl text-white text-xs sm:text-base flex justify-center' >
                          Book Now
                  </button>
                </Link>  
            </div>                          
